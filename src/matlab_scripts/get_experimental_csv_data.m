function [estrade, poo, achuth, brazier, qian, kuschel ] = get_experimental_csv_data()
...
%  Sommerfeld & Kuschel 2016 Exp Fluids
% ?Modelling droplet collision outcomes for different substances and viscosities
% ?DOI 10.1007/s00348-016-2249-y
outcome_names = [{'coalescence'}, {'bouncing'}, {'stretching'}, {'reflexive'}];
liq_names = [{'ethanol'}, {'propanol'}, {'hexanol'}, ...
    {'heptanol'}, {'nonanol'}, {'dodecanol'}, ...
    {'fva100'}, {'fva090'}, {'fva070'}, {'fva060'}, {'fva045'}, {'fva023'}, ...
    {'PVP_k30_ma05'}, {'PVP_k30_ma15'}, {'PVP_k30_ma25'}, ...
    {'sucrose_54'}, {'sucrose_58'}, ...
    ];
liq_visc = [1.2, 1.98, 4.3, ...
            7.4, 12.9, 15.9, ...
            3.04, 3.6, 5.3, 6.7, 14.5, 28.5, ...
            2.6, 12.5, 60, ...
            19.4, 42.2]*10^(-3);
fig_names = [{'5a'}, {'5b'}, {'5c'}, {'5d'}, {'5e'}, {'5f'},...
             {'6a'}, {'6b'}, {'6c'}, {'6d'}, {'6e'}, {'6f'}, ...
             {'15a'}, {'15b'}, {'15c'}, ...
             {'16a'}, {'16b'}, ...
            ];

kuschel.liq_names = liq_names;
kuschel.liq_visc  = liq_visc;

for i = 1:length(liq_names)
    filename = ['kuschel_fig', fig_names{i}, '.csv'];
    ref_path = [directory_path, filename];
    T = readtable(ref_path);
    A_tmp = table2cell(T);
    clearvars A
    ...
    tmp = A(:, 1);
    kuschel.(liq_names{i}).coalescence.x = cellfun(@str2num, tmp(~strcmp(tmp, '')));
    tmp = A(:, 2);
    kuschel.(liq_names{i}).coalescence.y = cellfun(@str2num, tmp(~strcmp(tmp, '')));
    tmp = A(:, 3);
    kuschel.(liq_names{i}).bouncing.x = cellfun(@str2num, tmp(~strcmp(tmp, '')));
    tmp = A(:, 4);
    kuschel.(liq_names{i}).bouncing.y = cellfun(@str2num, tmp(~strcmp(tmp, '')));
    tmp = A(:, 5);
    kuschel.(liq_names{i}).stretching.x = cellfun(@str2num, tmp(~strcmp(tmp, '')));
    tmp = A(:, 6);
    kuschel.(liq_names{i}).stretching.y = cellfun(@str2num, tmp(~strcmp(tmp, '')));
    tmp = A(:, 7);
    kuschel.(liq_names{i}).reflexive.x = cellfun(@str2num, tmp(~strcmp(tmp, '')));
    tmp = A(:, 8);
    kuschel.(liq_names{i}).reflexive.y = cellfun(@str2num, tmp(~strcmp(tmp, '')));
end

for i = 1:length(outcome_names)
    kuschel.all.(outcome_names{i}).x = [];
    kuschel.all.(outcome_names{i}).y = [];
end

for j = 1:length(liq_names)
    for i=1:length(outcome_names)
        kuschel.all.(outcome_names{i}).x   = [kuschel.all.(outcome_names{i}).x, kuschel.(liq_names{j}).(outcome_names{i}).x'];
        kuschel.all.(outcome_names{i}).y   = [kuschel.all.(outcome_names{i}).y, kuschel.(liq_names{j}).(outcome_names{i}).y'];
    end
end
